...
export const buildUtilsContent = (
  ...
  const libraryPath = relativeImportPath(config.paths.utils, config.paths.library);
  const schemasPath = relativeImportPath(config.paths.utils, config.paths.input);
  const typesPath = relativeImportPath(config.paths.utils, config.paths.types);

  const lines = headers().concat([
    "",
    `import { ${needsFactoryImport ? "coerceFactory, " : ""}coerceValue } from "${libraryPath}";`,
